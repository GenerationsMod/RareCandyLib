...
import generations.gg.rarecandylib.common.client.util.MatrixCache
...
import generations.gg.rarecandylib.common.client.providers.MaterialProvider
import generations.gg.rarecandylib.common.client.providers.TintProvider
import generations.gg.rarecandylib.common.client.texture.MissingTextureProxy
...
object Pipelines {
    val INSTANCE: RenderContext.Key<CobblemonInstance> = RenderContext.key("object_instance".asRclResource(), TypeToken.get(
        CobblemonInstance::class.java)) //TODO: either remove or properly use again.
    ...
    fun onInitialize(manager: ResourceManager) {
        if (!initialized) {
            ...
            initialized = true
            useLegacy = true //GenerationsCore.CONFIG.client.usePixelmonShading TODO: Decide if I want to hardcord shading to smooth shading or not
    ...
    fun createShader(manager: ResourceManager): Pipeline = Pipeline.Builder()
        ...
        .supplyTexture("diffuse", 0) { it.instance.instanceOrNull<MaterialProvider>()?.material?.let { MinecraftTextureLoader.getTextureOrNull(it) } ?: it.getTextureOrOther({ it.material.images().diffuse }) { ITextureLoader.instance().nuetralFallback } }
    ...
    fun Pipeline.Builder.shader(manager: ResourceManager, vertex: String, fragment: String): Pipeline.Builder =
        shader(read(manager, vertex.asRclResource()), read(manager, fragment.asRclResource()))
    ...
    private val UniformUploadContext.statueMaterial: String?
        get() = this.instance.instanceOrNull<MaterialProvider>()?.material?.takeIf { MinecraftTextureLoader.has(it) }
    ...
    private fun UniformUploadContext.getTextureOrOther(function: (UniformUploadContext) -> String?, supplier: () -> ITexture): ITexture = MinecraftTextureLoader.getTexture(function.invoke(this))?.takeUnless { texture -> texture === MissingTextureProxy } ?: supplier.invoke()
