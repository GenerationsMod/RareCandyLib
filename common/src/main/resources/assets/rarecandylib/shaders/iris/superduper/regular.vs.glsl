...
vec4 getVertexColor(vec3 surfaceNormal) {
    if(legacy) return vec4(1.0);

    vec3 lightDir0 = normalize(Light0_Direction);
    vec3 lightDir1 = normalize(Light1_Direction);
    float light0 = max(0.0, dot(lightDir0, surfaceNormal));
    float light1 = max(0.0, dot(lightDir1, surfaceNormal));
